...
//*****************************************************************************
//	Function copyRect(PGDI_RECT rc1, PGDI_RECT rc2) 
...
//*****************************************************************************
//	Function bitBlt(PGDI_RECT prc, i16 x, i16 y, i16 w, i16 h, pu8 bm, u16 rop)
//
...
//*****************************************************************************
//	Function point(PGDI_RECT rc, u16 x, u16 y)
...
//		rop			Raster operation. See GDI_ROP_xxx defines
//*****************************************************************************
void line(PGDI_RECT prc, i16 x1, i16 y1, i16 x2, i16 y2, u16 rop) {
...
//*****************************************************************************
//	Function rectangleEx(PGDI_RECT rc, u16 rop)
//	Draw rectangle
//	parameters:
...
//*****************************************************************************
//	Function drawText(PGDI_RECT prc, pu8 ptext, u16 style, u16 rop)
...
//		rop			Raster operation. See GDI_ROP_xxx defines
//*****************************************************************************
void drawText(PGDI_RECT prc, pu8 ptext, u16 style, u16 rop) {
	...
	for (i = 0; i < l; i++) {
		...
		if (c >= GDI_SYSFONT_OFFSET) {
			...
			ptx = ((pu8) font) + pos;
			bitBlt(prc, xp, 0, GDI_SYSFONT_WIDTH, GDI_SYSFONT_HEIGHT, ptx, rop);
...
void drawTextEx(i16 x, i16 y, pu8 ptext, u16 rop) {
	...
	for (i = 0; i < l; i++) {
		...
		if (c >= GDI_SYSFONT_OFFSET) {
			pos = (u16) (c - GDI_SYSFONT_OFFSET) * GDI_SYSFONT_BYTEWIDTH * GDI_SYSFONT_HEIGHT;
			ptx = ((pu8) font) + pos;
			bitBlt(NULL, xp, y, GDI_SYSFONT_WIDTH, GDI_SYSFONT_HEIGHT, ptx, rop);
			xp += GDI_SYSFONT_WIDTH;
...
//*****************************************************************************
//	Function drawWindow(PGDI_WINDOW pwin)
...
void	drawWindow(PGDI_WINDOW pwin) {
	...
	if (pwin->style & GDI_WINCAPTION) {
		...
		rc.h += 10;
		for (i = 0; i < 11; i++) {
			line(NULL,rc.x, rc.y + i, rc.x + rc.w, rc.y + i, GDI_ROP_COPY);
		}
		if (pwin->style & GDI_WINCLOSEICON) {
			bitBlt(&rc, rc.w - 9, 1, 10, 9, (pu8) closeBm, GDI_ROP_COPY);
			rt.w -= 11;
		} else {
			rt.w -= 1;
		}
		drawText(&rt,pwin->caption,pwin->style & GDI_WINCAPTION_MASK, GDI_ROP_XOR);
	}
	rectangleEx(&rc,GDI_ROP_COPY);
}
