...
//***********************************************************************
void PWM_Init(){
  /**** Enable clocks for PORTB & TIMER4********/
  ...
  /* Configure PORTB Pin7 (TIM4 OC2)  as alternate function output Push pull 50 MHz */
  PORTB->CRL |= 0xb0000000;  // B=1011 ==> mode=11 (Output mode, max speed 50MHz), 
...
***********************************************/
    ...
    TIMER4->ARR = 39999;    // Set Auto reload value (periode)
    TIMER4->PSC = 3;        // Set Prescalar value
    // Output Compare Mode, ENABLE Preload,PWM  mode1
    TIMER4->CCMR[0]  = 0x00006800; //bit15 (OC1CE: Output Compare 2 Clear Enable) = 0 => OC1Ref is not affected 
                               // by the ETRF Input
                               ...
                               //Bit10 (Output Compare 2 fast enable) = 0
                               //Bits(9:8) = 00 => CC2 channel is configured as output
                               
    TIMER4->EGR |= (1<<0);    //Bit0 UG (Update generation ENABLE) = 1 => Re-initialize the counter 
                            //and generates an update of the registers
    
    // Enable CH2 output and polarity active-high
    TIMER4->CCER = 0x00000010;   //bit4 CC2E (Capture/Compare2 output enable) =1 => channel 2 enabled; 
                               //bit5 CC2P Capture/Compare2 output polarity) =0 =>  active high
                               
    TIMER4->CR1 |= (1<<7);  // Bit7 ARPE (Auto-reload preload enable) = 1 => TIMx_ARR register is buffered
    TIMER4->CR1 |= (1<<0);  // Bit0 CEN (Counter enable) = 1 => ENABLE Timer counter     
}
//==========================================
void pc13_init(){
    unsigned int ra;
...
void kmain (void){
    ...
    TIMER4->CCR[1]  = duty; // start with duty = 100
...
void SysTick_handler(){
  ...
  TIMER4->CCR[1]  = duty;   // SET duty cycle
}