...
void PWM_Init(){
  ...
  // Enable alternate function clock for remapping . 
	...
	// befor remaping PA1 is the channel2 output
	//PORTA->CRL |= 0x000000b0; // PA1 alternate function output Push pull 50 MHz

  /* Configure PORTB Pin3 (TIM2 OC2)  as alternate function output Push pull 50 MHz */
  PORTB->CRL |= 0x0000A000;  // B=1011 ==> mode=11 (Output mode, max speed 50MHz), 
	...
	// remap PA1 to PB3 
	// config AFIO_MAPR Bits(9:8) TIM2_REMAP[1:0]
	AFIO->MAPR |= 3<<8; // Bits(9:8) = 11: Full remap (CH1/ETR/PA15, CH2/PB3, CH3/PB10, CH4/PB11)
...
***********************************************/
    ...
    TIMER2->CCMR[0]  = 0x00006800; //bit15 (OC1CE: Output Compare 2 Clear Enable) = 0 => OC1Ref is not affected 
                               // by the ETRF Input
                               //bits(14:12) OC1M ( Output Compare1 mode) = 110 => PWM mode1 
                               //Bit11 (Output Compare 1 preload enable) = 1 ==>
                               // TIMx_CCR0 preload value is loaded in the active register at each update event.
                               //Bit10 (Output Compare 1 fast enable) = 0
                               //Bits(9:8) = 00 => CC1 channel is configured as output
