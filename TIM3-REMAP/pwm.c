...
//***********************************************************************
void PWM_Init(){
  /**** Enable clocks for PORTA & TIMER3  ********/
  RCC->APB2ENR |= (1<<3) | (1<<2);     // enable PORTA and PORTB clocks
  RCC->APB1ENR |= (1<<1);     // enable clock for timer 3
  // Enable alternate function clock for remapping . 
	RCC->APB2ENR |= (1 << 0); // Bit0 = 1 ==> AFIO  enabled

	// befor remaping PA6 is the channel1 output
	//PORTA->CRL |= 0x0b000000; // PA6 alternate function output Push pull 50 MHz

  /* Configure PORTB Pin4 (TIM3 OC1)  as alternate function output Push pull 50 MHz */
  PORTB->CRL |= 0x000b0000;  // B=1011 ==> mode=11 (Output mode, max speed 50MHz), 
                            //cnf=10 (Alternate function output Push-pull)

	// we must disable JTAG and SW on PORTB
	AFIO->MAPR &= 0xF0FFFFFF; // ~(7<<24) reset bits(26:24)
	AFIO->MAPR |= 0x04000000; // 4<<24  bits(26:24) = 100 => JTAG-DP and SW-DP disabl 							

	// remap PA6 to PB4 
	// config AFIO_MAPR Bits(11:10) TIM3_REMAP[1:0]
	AFIO->MAPR |= 2<<10; // Bits(11:10) = 10: Partial remap (CH1/PB4, CH2/PB5)
...
***********************************************/
    ...
    TIMER3->ARR = 39999;    // Set Auto reload value (periode)
    TIMER3->PSC = 3;        // Set Prescalar value
    // Output Compare Mode, ENABLE Preload,PWM  mode1
    TIMER3->CCMR[0]  = 0x00000068; //bit7 (OC1CE: Output Compare 1 Clear Enable) = 0 => OC1Ref is not affected 
                               // by the ETRF Input
                               //bits(6:4) OC1M ( Output Compare1 mode) = 110 => PWM mode1 
                               //Bit3 (Output Compare 1 preload enable) = 1 ==>
                               // TIMx_CCR0 preload value is loaded in the active register at each update event.
                               //Bit2 (Output Compare 1 fast enable) = 0
                               //Bits(1:0) = 00 => CC1 channel is configured as output
                               
    TIMER3->EGR |= (1<<0);    //Bit0 UG (Update generation ENABLE) = 1 => Re-initialize the counter 
                            //and generates an update of the registers
    
    // Enable CH1 output and polarity active-high
    TIMER3->CCER = 0x00000001;   //bit0 CC1E (Capture/Compare2 output enable) =1 => channel 1 enabled; 
                               //bit1 CC1P Capture/Compare2 output polarity) =0 =>  active high
                               
    TIMER3->CR1 |= (1<<7);  // Bit7 ARPE (Auto-reload preload enable) = 1 => TIMx_ARR register is buffered
    TIMER3->CR1 |= (1<<0);  // Bit0 CEN (Counter enable) = 1 => ENABLE Timer counter     
}
//==========================================
void pc13_init(){ 
    unsigned int ra;
...
void kmain (void){
    ...
    TIMER3->CCR[0]  = duty; // start with duty = 100
...
void SysTick_handler(){
  ...
  TIMER3->CCR[0]  = duty;   // SET duty cycle
}